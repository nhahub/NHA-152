...
const BecomeSellerForm = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const res = await axios.post("http://localhost:5000/api/seller", formData);
    ...
    <section
      ...
      >
        <form onSubmit={handleSubmit} className="flex flex-col gap-6">
          ...
          <button
            type="submit"
            className="w-full text-white font-semibold py-3 rounded-lg shadow-md hover:opacity-90 transition-all duration-300 mt-2"
            style={buttonStyle}
          >
            Become a Seller
          </button>
